INVENTORY_PROMPT = """Generate a JSON output that catalogs the count of movable items along with the name of that item, this wil help
        movers and packers to estimate total items such as furniture items, wall mounted art works/paintings, also don't include small crockeries or very small kitchen items
        and electronics (Don't include built in Applications like fridge and dishwasher), don't include the items with count zero or that are not present, 
        ...
        inDb : true if in database_items else false, "volume": `approx volume in CFT(floating point)`, "weight": `approx weight in lbs(floating point)`  }} ] "room_name" : `name of the room eg(Living room, bedroom)` }}
